...
SELECT 
    genero, ROUND(AVG(duracao), 2) AS media_duracao
FROM
    filmes
GROUP BY genero;
...
SELECT 
    genero, ROUND(SUM(preco_aluguel), 2) AS soma_aluguel
FROM
    filmes
GROUP BY genero;
...
SELECT 
    id_filme, COUNT(id_filme) AS contagem
FROM
    alugueis
GROUP BY id_filme;
...
SELECT 
    id_cliente, ROUND(AVG(nota), 2) AS media_nota
FROM
    alugueis
GROUP BY id_cliente;
...
SELECT 
    data_aluguel, COUNT(*) AS contagem_alugueis
FROM
    alugueis
...
SELECT 
    ano_lancamento, MAX(preco_aluguel)
FROM
    filmes
GROUP BY ano_lancamento;